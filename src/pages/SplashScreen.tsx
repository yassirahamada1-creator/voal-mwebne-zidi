  ...
  return (
    <div className="fixed inset-0 flex h-[100dvh] max-h-[100dvh] w-screen flex-col items-center justify-between gradient-hero overflow-hidden overscroll-none touch-none px-4 pt-[max(env(safe-area-inset-top),0.75rem)] pb-[max(env(safe-area-inset-bottom),0.75rem)] sm:px-8 sm:pt-10 sm:pb-8">
      ...
      <button
        type="button"
        onClick={skipSplash}
        className="absolute top-[max(env(safe-area-inset-top),0.75rem)] right-4 z-20 text-xs font-medium rounded-full bg-primary-foreground/15 text-primary-foreground border border-primary-foreground/25 backdrop-blur-sm hover:bg-primary-foreground/25 shadow-lg font-sans py-[4px] px-px pb-[3px] pt-[3px] pr-0 pl-0"
      ...
      <div
        className={`relative z-10 flex flex-col items-center px-0${
      ...
      >
        ...
        <h1
          className="text-center font-display text-xl sm:text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-primary-foreground"
          style={{ animationDelay: "0.2s", animationFillMode: "both", lineHeight: "1.1" }}
        >
          {t.splash.appTitle}
        </h1>
        <p
          className="mt-1.5 sm:mt-3 max-w-[320px] text-center sm:text-[11px] md:text-xs font-semibold uppercase not-italic tracking-[0.2em] leading-snug text-orange-300 text-xs"
      ...
      <div
        className={`relative z-10 mb-2 sm:mb-6 flex w-full max-w-md sm:max-w-lg md:max-w-xl flex-col items-center gap-2${
          phase === "leaving" ? "opacity-0 translate-y-4" : ""
        }`}
