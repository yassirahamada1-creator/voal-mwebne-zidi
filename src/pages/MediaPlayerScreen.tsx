      ...
      <div className="relative w-full bg-foreground/90">
        {blockedOffline ? (
          <div className="aspect-video w-full flex flex-col items-center justify-center gap-3 px-6 text-center bg-foreground/95 text-primary-foreground">
            ...
            <p className="text-sm font-semibold">{t.pages.media.offlineTitle}</p>
            <p className="text-xs opacity-80 max-w-xs">{t.pages.media.offlineHint}</p>
          </div>
          ...
          <div className="mx-auto w-full max-w-[min(100%,calc(70vh*16/9))] bg-media-letterbox">
            <NativeVideo src={playableUrl} poster={posterUrl ?? undefined} ariaLabel={titleStr} className="w-full" />
          </div>
    ...
    >
      ...
      <div className="px-4 pt-4">
        {content.type === "text" || content.type === "image"
          ...
          : description && (
              <p className="mt-2 text-sm text-muted-foreground leading-relaxed whitespace-pre-wrap">{description}</p>
            )}
