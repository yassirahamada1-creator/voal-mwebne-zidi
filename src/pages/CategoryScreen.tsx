        ...
        <div className="relative px-4 pb-3">
          <div className="relative flex items-center">
            ...
            <input
              ...
              onFocus={() => query.length > 0 && setShowSuggest(true)}
              onBlur={() => setTimeout(() => setShowSuggest(false), 150)}
              placeholder={t.pages.category.searchPlaceholder}
              className="w-full rounded-full border border-border bg-muted py-2 pl-9 pr-9 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary"
            />
            {query && (
              <button
                type="button"
                ...
                className="absolute right-3 text-muted-foreground hover:text-foreground clear"
              >
                <X className="h-4 w-4" />
              </button>
            )}
          ...
          videos.map((item, i) => (
            <OfflineLock key={item.id} urls={[item.media_url, item.thumbnail_url]}>
              ...
              >
                ...
                {/* Photos liées à cette vidéo */}
                {photosByVideoId.has(item.id) && (
                  <div className="px-3 pb-3">
                    <PhotoStrip items={photosByVideoId.get(item.id)!} />
                  </div>
                )}

                {/* Récits et Audio liés à cette vidéo */}
                {contents
                  .filter((c) => (c.type === "recit" || c.type === "audio") && c.parent_id === item.id)
                  .map((child) => (
                    <div
                      key={child.id}
                      className="mx-3 mb-3 flex items-center gap-3 rounded-lg bg-muted px-3 py-2 cursor-pointer active:scale-[0.98] transition-transform"
                      onClick={() =>
                        navigate(`/media/${child.id}`, {
                          state: { fromPath: location.pathname },
                        })
                      }
                    >
                      <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-background overflow-hidden">
                        {child.thumbnail_url ? (
                          <OfflineImage src={child.thumbnail_url} alt="" className="h-full w-full object-cover" />
                        ) : (
                          <img
                            src={getFallbackThumbnail(child.type)}
                            alt=""
                            loading="lazy"
                            className="h-full w-full object-cover"
                          />
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        />
                        <span className="text-[11px] text-muted-foreground capitalize">{child.type}</span>
                      </div>
                    </div>
                  ))}
              </div>
