...
import { Search, Music, Leaf, UtensilsCrossed, Mic as Mic2, Shirt, Users, BookOpen, X, ChevronRight, Video, Headphones, FileText, Image as ImageIcon, Images, CircleHelp as HelpCircle } from "lucide-react";
...
const HomeScreen = () => {
  ...
  const safeModules = useMemo(() => modules.filter((m) => !BLOCKED_SLUGS.has(m.slug)), [modules]);
  ...
  return (
    <main className="flex flex-col bg-background pb-6">
      {/* Header sticky — colle en haut de la zone scrollable */}
      <header
        ...
        <div className="relative z-10">
          ...
          <div className="relative">
            ...
            <input
              ...
              aria-activedescendant={activeIndex >= 0 && showSuggestions ? optionId(activeIndex) : undefined}
        ...
        <div ref={sectionRef} className="mt-8 px-4">
          {loading ? (
            <div className="grid grid-cols-2 gap-4">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="card-cultural h-32 opacity-0" aria-hidden="true" />
              ))}
            </div>
              ...
              {filteredModules.map((mod) => {
                ...
                return (
                  <button
                    ...
                    className="card-cultural card-cultural-interactive group flex flex-col p-4 text-left"
                  >
                    {style.image ? (
                      <div className="mb-3 h-12 w-12 overflow-hidden rounded-xl shadow-md ring-1 ring-background/40">
                        <img
                          src={style.image}
                          alt=""
                      ...
                      <div
                        className={`mb-3 flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br ${style.gradient} shadow-md ring-1 ring-background/40`}
                      >
                    ...
                    <ChevronRight className="absolute right-3 top-3 h-4 w-4 text-muted-foreground/40 group-hover:text-primary" />
                  </button>
                );
              })}
            </div>
