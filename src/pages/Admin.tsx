...
import { Plus, Pencil, Trash2, ArrowLeft, Lock, LogOut, Eye, EyeOff, Save, RotateCcw, Search, Music, Video, FileText, Image as ImageIcon, Images, FolderPlus, LayoutDashboard, Languages, BookOpen, Upload, Loader as Loader2, X } from "lucide-react";
      ...
      <main className="container mx-auto px-4 py-6">
        <Tabs defaultValue="dashboard">
          <TabsList className="grid grid-cols-8 w-full mb-6">
            ...
            <TabsTrigger value="hommage">Hommage</TabsTrigger>
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map((s) => (
          <div
            key={s.label}
            className="group relative overflow-hidden rounded-2xl bg-white border border-slate-200/70 p-5 shadow-sm hover:shadow-md"
          >
                ...
                <div className="grid grid-cols-1 gap-2 max-h-[60vh] overflow-y-auto">
                  {modules.map((m) => (
                    <button
                      ...
                      className="flex items-center justify-between rounded-lg border-2 border-border bg-card p-4 text-left hover:border-primary hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                    >
                  ...
                  {(chosenModule === "galerie"
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() => startNew(t)}
                        className="flex flex-col items-center gap-2 rounded-lg border-2 border-border bg-card p-5 text-center hover:border-primary hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
                      >
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {visibleModules.map((m) => {
            ...
            return (
              <div
                key={m.id}
                className={`text-left rounded-lg border bg-card p-4 group ${
                  inactive ? "opacity-60 border-dashed" : "hover:border-indigo-300 hover:shadow-md"
                }`} > <div className="flex items-start justify-between gap-3"> <button onClick={() => setSelectedSlug(m.slug)} className="flex-1 min-w-0 text-left" > <p className="font-medium text-sm truncate">{m.name_fr}</p> <p className="text-xs text-muted-foreground truncate">{m.name_shk}</p> </button> <Badge variant="secondary" className="text-[10px] shrink-0"> {count} quiz </Badge> </div> <div className="mt-3 flex items-center justify-between gap-2"> <button onClick={() => setSelectedSlug(m.slug)} className="text-xs text-indigo-600 group-hover:underline" > Gérer les quiz → </button> <div className="flex items-center gap-2"> <label className="flex items-center gap-2 text-[11px] text-muted-foreground cursor-pointer"> <span>{inactive ? "Masqué" : "Visible"}</span> <Switch checked={m.is_active} disabled={togglingId === m.id} onCheckedChange={(v) => toggleModuleActive(m, v)} aria-label={`Rendre ${m.name_fr} ${m.is_active ? "invisible" : "visible"} dans l'application`} /> </label> <Button size="icon" variant="ghost" onClick={() => clearModuleQuizzes(m)} disabled={count === 0} title={ count === 0 ? "Aucun quiz à supprimer" : `Supprimer les${count}quiz du module` } aria-label={`Supprimer les quiz du module${m.name_fr}`} className="text-rose-600 hover:bg-rose-50 hover:text-rose-700 h-8 w-8" > <Trash2 className="w-4 h-4" /> </Button> </div> </div> </div> ); })} {visibleModules.length === 0 && ( <p className="text-sm text-muted-foreground col-span-full"> Aucun module disponible. Créez d'abord un module.
            </p>
          )}
        </div>
      </div>
